...
var global_backgroundLoaded = false;
...
chrome.runtime.onMessage.addListener(function (msg, sender, fnResponse) {
    if (msg === "requestRPAContentScript") {
        if (g_codeMap) {
...
var g_nativeMsgComm = function () {
    ...
    var m_returnMap = {};
    var nativeAppName = "com.rpa.msghost";
    ...
    var HandleFunctionCall = function (message) {
        if (typeof g_functionCallMap === "undefined") {
            console.error("HandleFunctionCall could not handle message.commandName: " + message.commandName);
            return;
        }
...
*/
        var requestedFunc = g_functionCallMap[message.commandName];
        if (requestedFunc !== undefined) {
            ...
            console.error("HandleFunctionCall: requested function is not available message.functionCall: " + message.commandName);
        }
    }
    ...
    m_nativeMsgPort.onMessage.addListener(function (message) {
        ...
        if (message.commandName) {
            ...
            if (message.commandName === "OnBridgeChanged") {
                HandleProductVersionChange(message);
            }
            else if (message.commandName === "IsPluginReady") {
                m_nativeMsgPort.postMessage({
                    retCode: 1,
                    returnId: message.requestId,
                    ready: global_backgroundLoaded
                });
                // console.log("slkdfjlsdfjsklf");
                return;
            }
            else {
                // 执行命令
    ...
    return {
        ...
        CallFunction: function (functionName, inputParams, returnFunc) {
            ...
            inputParams.commandName = functionName;
